...
export default function OperationsDashboard({ lang, onKnowledge, onActivity }: { lang: Lang; onKnowledge: () => void; onActivity?: (text: string) => void }) {
  ...
  return <section className="operations-dashboard">
    ...
    {result && <div aria-live="polite">
      ...
      <div className="section-heading"><h3>{bn ? "সময়ভিত্তিক তুলনা" : "Interval performance"}</h3><label>{bn ? "লাইনের রেকর্ড" : "Line records"}<select value={lineFilter} onChange={e => { setLineFilter(e.target.value); setPage(0); }}><option value="all">{bn ? "সব লাইন" : "All lines"}</option>{[...new Set(result.points.map(p => p.line))].map(line => <option key={line}>{line}</option>)}</select></label></div><p className="muted">{bn ? "ফিল্টার শুধু নিচের চার্ট ও টেবিলে প্রযোজ্য। সারাংশে সব রেকর্ড আছে।" : "Filter applies to the chart and table only. Summary cards and patterns include all submitted records. Bars show the first 24 matching intervals, capped at 100%."}</p>
      <div className="readiness-bars">{visiblePoints.slice(0,24).map((p,i) => <div className="chart-row" key={i}><span>{p.line} · {new Date(p.start).toLocaleString("en-GB", { timeZone:"Asia/Dhaka", day:"2-digit", month:"short", hour:"2-digit", minute:"2-digit" })}</span><span className="bar-track" aria-hidden="true"><span className={`bar-fill ${p.gap ? "bar-processing" : "bar-ready"}`} style={{width: `${p.target && p.actual !== null ? Math.min(100, p.actual / p.target * 100) : 0}%`}} /></span><strong>{p.actual ?? "—"}/{p.target}</strong></div>)}</div>
